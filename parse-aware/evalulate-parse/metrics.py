...
def meteor(candidate, references):
    ...
    for ref in reference_tokens:
        matches += sum(1 for w in candidate_tokens if w in ref)

    precision = matches / len(candidate_tokens) if candidate_tokens else 0
    recall = matches / sum(len(ref) for ref in reference_tokens) if reference_tokens else 0
    ...
    penalty = 0.5 * (len(candidate_tokens) / sum(len(ref) for ref in reference_tokens)) if reference_tokens else 0
